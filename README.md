...
`image_url` for each metal is generated from `FX_METAL_IMAGE_BASE_URL` when set; otherwise it falls back to `${FX_BASE_URL}/assets/metals/<symbol>.png` (e.g., `au.png`). If the URL is on `exchangerate.host` and `FX_API_KEY` is configured, the backend appends `?access_key=...` automatically.
