...
# Optional: metal icon base path used by API responses
FX_METAL_IMAGE_BASE_URL=https://api.exchangerate.host/assets/metals
...
`image_url` for each metal is generated from `FX_METAL_IMAGE_BASE_URL` when set; otherwise it falls back to `${FX_BASE_URL}/assets/metals/<symbol>.png` (e.g., `xau.png`). If the URL is on `exchangerate.host` and `FX_API_KEY` is configured, the backend appends `?access_key=...` automatically.
