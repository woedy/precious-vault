...
class PlatformSettingsView(viewsets.ViewSet):
    ...
    def update(self, request):
        ...
        convert_enabled = request.data.get('metals_convert_enabled')
        ...
        if convert_enabled is not None and not isinstance(convert_enabled, bool):
            return Response({'error': 'metals_convert_enabled must be a boolean'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if convert_enabled is not None:
            obj.metals_convert_enabled = convert_enabled
            fields_to_update.append('metals_convert_enabled')
